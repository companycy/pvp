...
class Pvpserver {
  ...
  def writePositionList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) {
    ret.+=((room.getCurrentPlayerCnt() - 1).toByte) // skip main player

    val newRet = new ArrayBuffer[Byte]()
    for (i <- room.playerInfoMap if i._2.userId != playerInfo.userId) {
  ...
  def writeRotationList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) {
    ret.+=((room.getCurrentPlayerCnt() - 1).toByte) // skip main player
  ...
  def writeStatusList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) {
    ret.+=((room.getCurrentPlayerCnt() - 1).toByte) // skip main player
  ...
  def writeStartedSkillList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) { // todo:
    ...
    newRet.+=(playerInfo.startedSkillArr.size.toByte)
    for (i <- playerInfo.startedSkillArr) {
      newRet.+=(i.charId)
      newRet.+=(i.skillId)
    }
    playerInfo.startedSkillArr.clear()

    // for (i <- room.playerInfoMap) { // if i._2.userId != userId) { // send skill to other
    ...
    // for (i <- room.playerInfoMap) { // if i._2.userId == userId) { // then clear skill list
    // i._2.startedSkillArr.clear()
    // }

    broadcast(room, newRet.toArray, playerInfo.getId())
  }

  def writeMpList(ret: ArrayBuffer[Byte], room: Room, userId: Int) {
    ret.+=((room.getCurrentPlayerCnt()).toByte) // all players' mp

    for (i <- room.playerInfoMap) {
      ret.+=(i._2.id)
      ret.++=(
  ...
  def writeBuffList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) {
    // ret.+=(playerInfo.getBufflistSize) // all buff list
   ...
   */
  def writeAppliedBuffList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) {
    // ret.+=(playerInfo.getAppliedBuffSize) //
  ...
  def writeJoystickPosList(ret: ArrayBuffer[Byte], room: Room, playerInfo: PlayerInfo) {
  ...
  def createRoom(userId: Int, recvedData: ByteBuffer): Array[Byte] = {
    ...
    val status: Byte = 0
    ret.+=(status)
    ret.++=(ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putInt(room.getId()).array())
    ret.+=(room.hostPlayerId)

    return ret.toArray
  }

  def broadcastJoinOrLeaveRoom(playerInfo: PlayerInfo, room: Room, msgType: Byte) = {
    val ret = new ArrayBuffer[Byte]()
    writeHeader(
      ret,
      0,
    ...
    ret.+=(room.maxPlayer)
    ret.+=(playerInfo.getId())
    ret.++=(playerInfo.name)
    ret.+=(0.toByte)
    broadcast(room, ret.toArray, playerInfo.getId())
  }

  def broadcast(room: Room, ret: Array[Byte], id: Byte = 0) = {
    for (i <- room.playerInfoMap if i._2.getId() != id) {
  ...
  def broadcastStartgame(userId: Int, room: Room) = {
    ...
    for (i <- room.playerInfoMap) {
      ret.+=(i._2.getId())
      // ret.+=(1) // todo: for test
      ret.+=(i._2.getId()) // work as spawn point
    }
  ...
  def joinRoom(userId: Int, recvedData: ByteBuffer): Array[Byte] = {
    ...
    for (i <- room.playerInfoMap if i._2.getId() != playerInfo.getId()) {
      ret.+=(i._2.getId())
      ret.++=(i._2.name)
      ret.+=(Byte.box(0))
